...
{
	if(buffer == NULL)
		BAD_ERROR("put_write_buffer_hash: NULL buffer\n");
	if(buffer->buffer_type != QUEUE_CACHE)
		BAD_ERROR("put_write_buffer_hash: unexpected buffer type %d, buffer %p\n",
			buffer->buffer_type, (void *) buffer);
	if(buffer->queue_cache == NULL)
		BAD_ERROR("put_write_buffer_hash: NULL queue_cache, buffer %p\n",
			(void *) buffer);
	if(buffer->thread >= buffer->queue_cache->threads)
		BAD_ERROR("put_write_buffer_hash: invalid thread %d (threads %d), buffer %p\n",
			buffer->thread, buffer->queue_cache->threads, (void *) buffer);
	...
	struct file_buffer *read_buffer, int *duplicate_file, int bl_hash)
{
	...
	if(block_dup == FALSE) {
		for(block = thresh; block < blocks; block ++)
			if(buffer_list[block]) {
				if(buffer_list[block]->buffer_type != QUEUE_CACHE)
					BAD_ERROR("write_file_blocks_dup: non-QUEUE_CACHE buffer at block %d, type %d, ptr %p\n",
						block, buffer_list[block]->buffer_type,
						(void *) buffer_list[block]);
	...
	struct file_buffer *read_buffer, int *dup)
{
	...
	/*
	 * Synthetic sparse placeholders generated by -min-sparse-copy can make
	 * many files look structurally identical.  Skip duplicate pre-check in
	 * that mode to avoid driving the duplicate path with transient buffers.
	 */
	if(min_sparse_copy == 0 &&
			pre_duplicate(read_size, dir_ent->inode, read_buffer, &bl_hash))
...
int main(int argc, char *argv[])
{
	...
	for(i = option_offset; i < argc; i++) {
		if(strcmp(argv[i], "-no-pager") == 0)
			...
			sparse_files = FALSE;
		else if(strncmp(argv[i], "--min-sparse-copy=", 18) == 0) {
			long long number = 0;
			if(!parse_numberll(argv[i] + 18, &number, 1))
